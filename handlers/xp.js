...
class Util {
  ...
  static addexp(message) {
    ...
    let custms = db.get(`custms_${message.guild.id}`);
    if (custms === null) custms = ':clap: :partying_face: llegaste al nivel';
    
    if(newlvl > oldlvl) message.channel.send(`${message.author}, ${custms} ${newlvl}`)
